...
const loadStatistics = async () => {
  try {
    ...
    const response = await axios.get('vcards/' + userStore.userId + '/statistics')
...
const setCategoryChartData = () => {
  return {
    labels: statistics.value.transactions_by_category.map((item) => item.category),
    datasets: [
      {
        label: 'Gastos',
        data: statistics.value.transactions_by_category.map((item) => item.total_debit),
        fill: false
      },
      {
        label: 'Ganhos',
        data: statistics.value.transactions_by_category.map((item) => item.total_credit),
        fill: false
      }
    ],
  }
...
onMounted(async () => {
  ...
  categoryChart.value = setCategoryChartData()
  ...
  <div class="statistics-container" v-if="!isLoading">
    ...
    <div class="charts-container">
      <TabView class="statistics-tabs-container">
        ...
        <TabPanel header="Gastos/Ganhos por Categoria">
          <Chart type="bar" :data="categoryChart" :options="chartOptions" />
        </TabPanel>
      </TabView>
    </div>
  </div>
